...
class BasicModelUtil(ModelUtilAbstract):

    # 不输出的字段
    modelFieldExclude = ['status',]
    # 不显示的字段
    modelShowFieldExclude = ['status',]
    # 不显示的字段类型
    modelFieldTypeExclude = ['id','status',]
    ...
    def getChoicesFieldDict(self):
        ...
        if hasattr(self.modelobj, 'ChoicesField'):
            ...
            for field in ModelChoicesFieldDict:
                choicesFieldList = []
                modeldemo = BasicModelUtil(ModelChoicesFieldDict[field]['model'])
