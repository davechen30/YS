    ...
    <div class="con" style="height:100%">
      ...
      <div class="button-nav-container" style="margin-top: 5px;">
        ...
        <el-button
          ...
          >批量删除</el-button>
        <!-- <el-button class="button-nav" @click="downloadExcel">下载Excel</el-button>
        <el-button class="button-nav" @click="downloadExcel">下载Excel模板</el-button>
        <el-button class="button-nav">上传Excel</el-button> -->
      </div>
      <!-- button-nav -->
      <!-- container -->
    ...
    handleSelectionChange(val) {
      ...
      for (var key in this.multipleSelection[0]) {
        if (this.data_field_type[key]==='BooleanField') {
          if (this.multipleSelection[0][key]=="是") {
            this.update_form[key] = true
          }else {
            this.update_form[key] = false
          }
        }else {
...
//下载excel
    downloadExcel() {
      ...
      .then((res)=>{
        ...
        if (window.navigator.msSaveOrOpenBlob) {
               //兼容ie
            window.navigator.msSaveBlob(blob, "test.xls");
        } else {
  ...
  watch:{
    '$route'(to,from){
      ...
      this.model = to.params.modelname
  ...
  mounted() {
    ...
    this.model = this.$route.params.modelname
    this.lowermodel = this.model.toLowerCase()
    var permissionDict = this.$store.getters.permissionDict[this.lowermodel]
    for (var value in permissionDict) {
      this.butShow[value] = permissionDict[value]
